...
Raw runtime startup enables all mouse tracking modes except on WSL/ConPTY. The
TTY runtime does not enable mouse reporting. For a standalone Raw terminal
integration, the low-level API is:

```elixir
# Direct Raw setup (Runtime already owns this when using Raw)
...
Text pasted from clipboard when the terminal/host has bracketed paste mode
enabled. The 1.0 runtime parser supports these sequences but does not enable
the reporting mode automatically.
...
Host-created timing metadata events.
...
`Event.Tick` can be constructed and injected by a host. `Command.timer/2` and
`Command.interval/2` deliver their configured messages directly; they do not
create `%Event.Tick{}` values.
