defmodule TermUI.Widgets.Label do
  @moduledoc """
  ...
  def render(props, area) do
    ...
    ellipsis = CharacterSet.current_charset().ellipsis
    ...
    cells =
      ...
      |> Enum.flat_map(fn {line, y} ->
        if y < area.height do
          render_line(line, y, area.width, align, truncate, style, ellipsis)
        else
          []
        end
      end)
  ...
  defp render_line(text, y, width, align, truncate, style, ellipsis) do
    # Process the text for display
    display_text =
      if truncate && String.length(text) > width do
        do_truncate(text, width, ellipsis)
      else
        text
      end
  ...
  defp do_truncate(text, width, _ellipsis) when width <= 3 do
    String.slice(text, 0, width)
  end

  defp do_truncate(text, width, ellipsis) do
    if String.length(text) > width do
      String.slice(text, 0, width - 1) <> ellipsis
    else
      text
    end
  end
