defmodule TermUI.Widgets.PickList do
  @moduledoc """
  ...
  def render(state, area) do
    ...
    # Render border
    cells =
      cells ++ render_border(title, modal_x, modal_y, modal_width, modal_height, style, chars)
  ...
  defp render_border(title, x, y, width, height, style, chars) do
    cells = []
    ...
    cells = cells ++ [positioned_cell(x, y, chars.tl, style)]

    cells =
      cells ++ for(i <- 1..(title_start - 1), do: positioned_cell(x + i, y, chars.h_line, style))
    ...
    cells =
      cells ++
        for(i <- title_end..(width - 2), do: positioned_cell(x + i, y, chars.h_line, style))

    cells = cells ++ [positioned_cell(x + width - 1, y, chars.tr, style)]
    ...
    cells =
      (cells ++
         for row <- 1..(height - 2) do
           [
             positioned_cell(x, y + row, chars.v_line, style),
             positioned_cell(x + width - 1, y + row, chars.v_line, style)
           ]
         end)
      |> List.flatten()

    # Bottom border
    cells = cells ++ [positioned_cell(x, y + height - 1, chars.bl, style)]

    cells =
      cells ++
        for(
          i <- 1..(width - 2),
          do: positioned_cell(x + i, y + height - 1, chars.h_line, style)
        )

    cells = cells ++ [positioned_cell(x + width - 1, y + height - 1, chars.br, style)]
