if Code.ensure_loaded?(GenStage) do
  ...
  defmodule TermUI.Widgets.StreamWidget.Consumer do
    @moduledoc """
    Optional GenStage adapter for `TermUI.Widgets.StreamWidget`.

    Add `:gen_stage` to the host application to enable this adapter. The
    StreamWidget direct push API does not require GenStage.
    """

    @error {:missing_dependency, :gen_stage}

    @spec start_link(pid(), keyword()) :: {:error, {:missing_dependency, :gen_stage}}
    def start_link(_widget_pid, _opts \\ []), do: {:error, @error}

    @spec subscribe(GenServer.server(), GenServer.server(), keyword()) ::
            {:error, {:missing_dependency, :gen_stage}}
    def subscribe(_consumer, _producer, _opts \\ []), do: {:error, @error}
  end
end
